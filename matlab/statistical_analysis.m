...
% Import saved data, sequence of random mu_d, collision time and duration
load('randomized_data_no_collision')
...
for k=1:n_samples
    ...
    for i=a:t/h
        ...
        mu_d = mu_d_seq(:,k);
        t_collision = t_collision_seq(k);
        coll_dur = coll_dur_seq(k);
        ...
        if isUnbiased == 0
            ...
            u(:,i+1) = 250*eye(2)*(mu_d-mu(:,i)) + [100, 0; 20, 0]*(0-mu_p(:,i)) + Ki*(I_term);
    ...
    stat_data = stepinfo(q(1,:)- q(1,1),time_vec,mu_d(1) - q(1,1),'SettlingTimeThreshold',0.005);
    overshoot(k) = stat_data.Overshoot;
    t_settling(k) = stat_data.SettlingTime;
    if isnan(t_settling)
        t_settling(k) = t - t_collision;
        fprintf('Nan found')
    else
        t_settling(k) = t_settling(k)- t_collision;
    end
    E_ss(k) = sum(abs(mu_d - q(:,end)))/2; % steady state error considering two joints
    RMSE(k) = (sqrt(mean((q(1,:) - mu(1,:)).^2)) + sqrt(mean((q(2,:) - mu(2,:)).^2)))/2;
end

% Compute average statistics
fprintf('Results\n')
avg_e_ss = sum(E_ss)/n_samples
avg_overshoot = sum(overshoot)/n_samples
avg_RMSE = sum(RMSE)/n_samples
avg_t_settling = sum(t_settling)/n_samples
